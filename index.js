...
const boxContent = (content, contentWidth, options) => {
  ...
  const colorizeContent = content =>
    options.backgroundColor ? getBGColorFn(options.backgroundColor)(content) : content;
  ...
  if (options.float === 'center') {
    const marginWidth = Math.max(
      (columns - contentWidth - getBorderWidth(options.borderStyle)) / 2,
      0
    );
    marginLeft = PAD.repeat(marginWidth);
  } else if (options.float === 'right') {
    const marginWidth = Math.max(
      columns - contentWidth - options.margin.right - getBorderWidth(options.borderStyle),
      0
    );
    marginLeft = PAD.repeat(marginWidth);
  }
...
const determineDimensions = (text, options) => {
  ...
  const widest =
    widestLine(wrapAnsi(text, columns - borderWidth, { hard: true, trim: false })) +
  ...
  if (!widthOverride) {
    ...
    options.width = Math.min(
      options.width,
      columns - borderWidth - options.margin.left - options.margin.right
    );
  }
