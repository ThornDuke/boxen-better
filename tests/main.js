...
test('box not overflowing terminal', t => {
  const box = boxenb('foo'.repeat(process.env.COLUMNS));
...
test('handles formatted text', t => {
  const box = boxenb(formattedText);

  t.snapshot(box);
});

test('handles random text', t => {
  const box = boxenb(randomText);

  t.snapshot(box);
});

test('handles colored texts', t => {
  let box = boxenb(chalk.red(longText));

  t.snapshot(box);

  box = boxenb(chalk.blue(formattedText));

  t.snapshot(box);

  box = boxenb(chalk.yellow(randomText));
